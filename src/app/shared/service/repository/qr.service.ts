...
export class QRService extends BaseService {
  ...
  getbyGuid(guid?: string): Observable<any> {
    ...
    return this.http
      .get<any>(this.apiURL + '/v1', { params: params })
      .pipe(retry(1));
  }
  ...
  getbyToken(token?: string): Observable<any> {
    ...
    return this.http
      .get<any>(this.apiURL + '/v1/gbt', { params: params })
      .pipe(retry(1));
  }
  ...
  sendEditEmail(guid?: string): Observable<any> {
    ...
    return this.http
      .get<any>(this.apiURL + '/v1/SendEmail', { params: params })
      .pipe(retry(1));
  }

  sendEditEmailCoord(guid: string, lat: number, lon: number): Observable<any> {
    let params: HttpParams = new HttpParams();
    if (
      guid !== undefined &&
      guid !== null &&
  ...
  CreatePetQrInformation(
    guid: string,
    petQrInformation: PetQRInfotmation
  ): Observable<any> {
    let json = {
      guid: guid,
  ...
  generate(count: number): Observable<any> {
    return this.http
      .post<any>(this.apiURL + '/v1', JSON.stringify(count), this.httpOptions)
      .pipe(retry(1));
  }
}
